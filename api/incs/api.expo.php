...
if (isset($_GET['id']) && !empty($_GET['id'])) {
	...
	if (@mysql_num_rows($sql) > 0) {
		...
		if ($don['private'] == $_GET['priv']) {
			...
			$item = array();
			$sql = @mysql_query("SELECT `id` FROM `items` WHERE `expo_id`='".mysql_real_escape_string($_GET['id'])."'");
			...
			while ($it = @mysql_fetch_assoc($sql)) {
				...
				if (is_file('../'.$main_file)) {
					$it['fichier'] = $config['site_http'].'/'.$main_file;
					$it['fichier_md5'] = $md5_main_file;
				} else {
					$it['fichier'] = '';
					$it['fichier_md5'] = '';
				}
		        ...
		        $sql2 = @mysql_query("SELECT ft.slug, itf.content FROM fields_type ft, expo_fields ef, item_fields itf WHERE ft.id = ef.fields_type_id AND ef.id = itf.expo_fields_id AND itf.items_id = '".$it['id']."';");
		        while ($don2 = @mysql_fetch_assoc($sql2)) {
			        //print_r($don2);
			        //$it[$don2['slug']] = $don2['content'];
			        $info_array[$don2['slug']] = urlencode($don2['content']);
		        }

		        // on ecrit ceux ci en dur car ils sont "a part":
		        $info_array['id'] = urlencode($it['id']);
		        $info_array['fichier'] = urlencode($it['fichier']);
		        $info_array['fichier_md5'] = urlencode($it['fichier_md5']);
				...
				$item[] = array(
				    'id' => urlencode($it['id']), 
				    'nom' => urlencode(@$it['nom']), 
				    'date_acqui' => urlencode(@$it['date_acqui']), 
				    'date_crea' => urlencode(@$it['date_crea']), 
				    'datation' => urlencode(@$it['datation']), 
				    'orig_geo' => urlencode(@$it['orig_geo']), 
				    'orig_geo_prec' => urlencode(@$it['orig_geo_prec']), 
				    'taille' => urlencode(@$it['taille']), 
				    'cartel' => urlencode(@$it['cartel']), 
				    'freefield' => urlencode(@$it['freefield']), 
				    'fichier' => urlencode($it['fichier']), 
				    'fichier_md5' => urlencode($it['fichier_md5']), 
				    'data' => $data, 
				    'keywords' => $kw
				);
				*/
