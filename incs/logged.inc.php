...
if(isset($_SESSION['user']) && !empty($_SESSION['user']) && isset($_SESSION['pass']) && !empty($_SESSION['pass'])) {
	...
	//$sql = @mysqli_query("SELECT `id`, `lastname`, `firstname`, `is_admin`, `mailaddress`, `password` FROM `users` WHERE `mailaddress`='".mysqli_real_escape_string(strtolower($_SESSION['user']))."' AND `password`='".mysqli_real_escape_string($_SESSION['pass'])."' LIMIT 0,1");
