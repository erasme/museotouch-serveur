...
if (isset($_GET['search']) && !empty($_GET['search'])) {
	$search = "AND `nom` LIKE '%".mysql_real_escape_string($_GET['search'])."%'";
}
...
$sql = @mysql_query("SELECT `id`, `nom` FROM `items` WHERE `expo_id`='".mysql_real_escape_string($_GET['expo'])."' $search ORDER BY `nom` LIMIT $from, $limit_page");

if (@mysql_num_rows($sql) >0) {
	while ($don = @mysql_fetch_assoc($sql)) {
		echo '<tr class="height-20">
			<td class="tablerow1">'.$don['nom'].'</td>
